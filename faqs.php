<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Frequently Asked Questions</title>
    <link rel="icon" type="image/x-icon" href="/images/cmulogo.png">
    <link rel="stylesheet" href="style/faqs.css">
    <script type="text/javascript" src="faqs.js" defer></script>
</head>
<body>
    <main id="content">
    <div class="back-home">
    <button onclick="location.href='/user_folder/reservation.php'">Back to Home</button>
</div>
        <div class="title">
            <h1> Frequently Asked Questions</h1>
        </div>
        

        <div class="container">
            <h2>Reservation Application Form Questions</h2>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>How do I make a reservation?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>You can typically make reservations online through the university's online reservation system or by contacting the facilities management office directly.</p></li>
            </ul>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>Who can submit a reservation request?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>Both CMU Professors and Admins can submit reservation requests.</p></li>
            </ul>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>What information do I need to provide?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>You will need to provide detail such as your name, year, section, and the time and date of your reservation.</p></li>
            </ul>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>Can I physically visit my preferred room prior to submitting a reservation?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>Yes, you may visit the room before making a reservation. Please coordinate with the facility manager.</p></li>
            </ul>
        </div>

        <div class="container">
            <h2>Reservation Processing Questions</h2>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>How will I know when my reservation is confirmed?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>Once your reservation is confirmed it will send a pop-up message that your "Reservation is Success".</p></li>
            </ul>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>What is the best way to contact CMU if I have a question about a reservation request?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>For any inquiries, please email your concern to <a href="#">Cityofmalabonuniversity@edu.ph</a>.</p></li>
            </ul>
        </div>

        <div class="container">
            <h2>Cancellation Policy</h2>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>What happens if I need to cancel my reservation?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>You can usually cancel your reservation by contacting the facilities management office or through the online reservation system.</p></li>
            </ul>

            <button onclick="toggleSubMenu(this)" class="dropdown-btn">
                <span>What if I need to reschedule my reservation?</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-compact-down" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M1.553 6.776a.5.5 0 0 1 .67-.223L8 9.44l5.776-2.888a.5.5 0 1 1 .448.894l-6 3a.5.5 0 0 1-.448 0l-6-3a.5.5 0 0 1-.223-.67"/>
                </svg>
            </button>
            <ul class="sub-menu ">
                <li><p>You can typically reschedule your reservation by contacting the facilities management office or through the online reservation system, but it's best to do so as soon as possible.</p></li>
            </ul>
        </div>
    </main>

</body>
</html>
