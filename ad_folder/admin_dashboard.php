...
$pending_count = 0;
if ($feature_enabled) {
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM registration WHERE is_active = 0");
    $stmt->execute();
...
<div class="row mb-4">
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <p>Verify new user accounts using OTP verification.</p>
                
                <?php if ($feature_enabled): ?>
                    <p>
                        <span class="badge bg-warning"><?php echo $pending_count; ?></span> 
                        account(s) pending verification
                    </p>
                    <a href="verify_accounts.php" class="btn btn-primary">Manage Verifications</a>
                <?php else: ?>
                    <div class="alert alert-info">
                        The verification feature needs to be enabled first. 
                        New registrations will automatically use this feature.
                    </div>
                <?php endif; ?>
            </div>
        </div>
