...
<ul class="sub-menu">
<li><a href="/ad_folder/dashboard.php">&nbsp;&nbsp;&nbsp;Dash Board</a></li>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    ...
    if ($action == 'update') {
        ...
        $stmt = $conn->prepare("UPDATE reservation SET name=?, purpose=?, roomno=?, reservation_time=?, reservation_date=? WHERE idreservations=?");
        $stmt->bind_param("sssssi", $name, $purpose, $roomno, $reservation_time, $reservation_date, $idreservations);
        $stmt->execute();
    ...
    if ($action == 'delete') {
        $idreservations = sanitize_input($_POST['idreservations']);
        $stmt = $conn->prepare("DELETE FROM reservation WHERE idreservations=?");
...
$sql = "SELECT idreservations, name, purpose, roomno, reservation_time, reservation_date FROM reservation";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
        echo "<table><tr><th>Reservation ID</th><th>Name</th><th>Purpose</th><th>Room No</th><th>Reservation Time</th><th>Reservation Date</th><th>Actions</th></tr>";
        while ($row = $result->fetch_assoc()) {
            ...
            <td>
                ...
                <form method='POST' action='admin_confirm.php' style='display:inline-block;'>
                    <input type='hidden' name='idreservations' value='{$row['idreservations']}'>
